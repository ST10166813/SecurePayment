...
// GET - list payments employee can act on (pending + verified)
router.get('/payments', requireAuth, requireRole('employee'), async (req, res, next) => {
  try {
    // Only show payments that are NOT yet submitted
    const payments = await Payment.find({ status: { $ne: 'submitted' } }) 
      .sort({ createdAt: -1 })
      .limit(200)
